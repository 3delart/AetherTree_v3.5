...
| Immobilisation explicite pendant la canalisation ? | **Non — pas de code ajouté.** Décision maintenue, mais sa justification d'origine ("`HandleAttack()`/`HandleCombatAI()` ne repositionnent pas l'agent dans cet état") s'est révélée **factuellement fausse dans 2 cas, trouvés en review finale** : (1) un PNJ dont la cible sort de `attackRange` en pleine canalisation retombe dans la branche `else` de `HandleCombatAI()` qui appelle `_agent?.SetDestination(...)` sans jamais consulter `_isChanneling` ; (2) un Mob **taunté** court-circuite `TryUseSkill()` entièrement dans `HandleChase()` (le taunt force l'attaque de base uniquement) et retombe sur `agent.SetDestination()`. Dans les deux cas, le Mob/PNJ marche pendant sa canalisation — cosmétique (la barre suit correctement puisqu'elle suit le `transform`), mais le `vfxCast` reste fixe au point de lancement et se retrouve visuellement décalé du caster en mouvement. Décision maintenue telle quelle (pas de fix de code) car le coût d'un vrai seuil de mouvement/immobilisation dépasse le bénéfice pour un défaut purement visuel — mais Florian doit s'attendre à le voir au test (point 9 de la Vérification) plutôt que le prendre pour un bug. |
